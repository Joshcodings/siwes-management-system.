...
const insert = db.prepare(`
  INSERT OR IGNORE INTO companies (name, email, industry_type, required_skills, address, latitude, longitude, allowed_radius)
  VALUES (?, ?, ?, ?, ?, ?, ?, 500)
`);

const insertMany = db.transaction(() => {
  for (const c of companies) {
    insert.run(c.name, c.email, c.industry_type, JSON.stringify(c.skills), c.address, c.lat, c.lon);
  }
});
...
console.log(`✅ Done! Total companies in database: ${count.count}`);
db.close();
